...
//	srand(time(0));
	...
	int x, *max = &x;
	Up_Down(first, max);
	cout << "max by up_down is " << *max << endl;
	cout << endl;

	cout << "find max by left_right" << endl;
	int y, *may = &y;
	Left_Right(first, may);
	cout << "max by left_right is " << *may << endl;
	cout << endl;

	cout << "find max by down_up" << endl;
	int z, *maz = &z;
	Left_Right(first, maz);
	cout << "max by down_up is " << *maz << endl;
	cout << endl;

	cout << "checking search element: " << val << endl;
	TreeNode *temp = t_search(first, val);
	if (temp != nullptr)
		cout << "we can find this element" << endl;
	else
